...
class Trainer(BaseTrainer):
    ...
    def _train_epoch(self, epoch):
        ...
        for batch_idx, (images_id, images, cap_lens, reports_ids, reports_masks, mesh_label) in enumerate \
            ...
            _, predicted = torch.max(kmve_output.data, 1)
            pred_output, _ = self.model(images, mode='sample')

            predcit_reports = '.'.join(
                self.model.tokenizer.decode_batch(pred_output.cpu().numpy()))
            ground_truths = '.'.join(self.model.tokenizer.decode_batch(reports_ids[:, 1:].cpu().numpy()))
            embeddings1 = self.sentence_bert.encode(predcit_reports, convert_to_tensor=True)
            embeddings2 = self.sentence_bert.encode(ground_truths, convert_to_tensor=True)
            cosine_scores = util.cos_sim(embeddings1, embeddings2).item()
            CS_L = cosine_scores
            if CS_L < 0:
                CS_L = 0
            CS_L = self.logloss(y_true=[1], y_pred=[CS_L])

            # SF_LOSS
            KMVE_l = self.criterionBCE(kmve_output, mesh_label)
            RG_L = self.criterion(output, reports_ids, reports_masks)
            total_loss = KMVE_l + RG_L + CS_L
            train_loss += KMVE_l.item()
            train_loss += RG_L.item()
            train_loss += CS_L

            self.optimizer.zero_grad()
            total_loss.backward()

            torch.nn.utils.clip_grad_value_(self.model.parameters(), 0.1)  # 对梯度进行按值剪裁
            self.optimizer.step()
        ...
        with torch.no_grad():
            ...
            for batch_idx, (images_id, images, cap_lens, reports_ids, reports_masks, mesh_label) in enumerate(
                ...
                reports = self.model.tokenizer.decode_batch(output.cpu().numpy())
                ground_truths = self.model.tokenizer.decode_batch(reports_ids[:, 1:].cpu().numpy())
                df = df.append({'key': images_id, 'gt': ground_truths, 'pred': reports, 'TestAcurracy': 0},
                               ignore_index=True)
